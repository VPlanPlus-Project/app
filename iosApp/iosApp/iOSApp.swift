...
struct iOSApp: SwiftUI.App {
    ...
    init() {
        guard !isRunningInPreview else {
            self.quickLookImpl = QuickLookImpl()
            self.sheetCoordinator = IosDevInfoSheetCoordinator()
            return
        }
        
        quickLookImpl = QuickLookImpl()
        sheetCoordinator = IosDevInfoSheetCoordinator()

        let koinApplication = MainViewControllerKt.doInitKoin(
            quicklookImpl: quickLookImpl,
            iosDevInfoSheetHandlerImpl: sheetCoordinator
        )
        
        
        KoinHelper.shared.install(into: koinApplication, detailDrawerLauncher: SwiftGradeDetailDrawerLauncher())
    }
...
class AppDelegate: NSObject, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication,
        ...
        guard !isRunningInPreview else { return true }
