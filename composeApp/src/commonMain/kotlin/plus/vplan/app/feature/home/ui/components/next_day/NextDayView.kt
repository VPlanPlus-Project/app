...
fun NextDayView(day: HomeViewDay) {
    Column(
        ...
        Column lessons@{
            val lessonTimes by combine(day.timetable.map { it.lessonTime }.distinct().map { App.lessonTimeSource.getById(it).filterIsInstance<CacheState.Done<LessonTime>>().map { it.data } }) { it.toList() }.collectAsState(emptyList())
            if (lessonTimes.isEmpty()) return@lessons
            SectionTitle(
                modifier = Modifier.padding(horizontal = 16.dp),
                title = "Nächste Stunden",
                subtitle =
                    if (day.day.substitutionPlan.ifEmpty { day.timetable }.isEmpty()) "Keine Stunden"
            ...
            FollowingLessons(
                ...
                date = day.day.date,
                lessons = day.timetable.groupBy { l -> lessonTimes.first { it.id == l.lessonTime }.lessonNumber }
            )
        }
    }
}