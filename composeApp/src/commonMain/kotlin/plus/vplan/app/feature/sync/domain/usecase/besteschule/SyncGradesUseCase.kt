...
class SyncGradesUseCase(
    ...
    private val besteSchuleSyncCoordinator by inject<BesteSchuleSyncCoordinator>()
    ...
    suspend operator fun invoke(allowNotifications: Boolean, yearId: Int? = null) {
        ...
        vppIds.forEach forEachUser@{ user ->
            val yearIdsToRunUpdatesFor = mutableListOf<Int>()

            if (yearId == null) {
                besteSchuleSyncCoordinator.syncYears(user.schulverwalterConnection!!.userId)
                yearIdsToRunUpdatesFor.addAll(besteSchuleYearsRepository.getYearsFromCache().first().map { it.id })
            } else {
                yearIdsToRunUpdatesFor.add(yearId)
            }

            yearIdsToRunUpdatesFor.forEach forEachYear@{ yearId ->
                val syncResult = besteSchuleSyncCoordinator.syncBesteSchule(
                    schulverwalterUserId = user.schulverwalterConnection!!.userId,
                    yearId = yearId
                )

                val gradesEligibleForNotification = syncResult.newGrades
                    .filter {
                        it.givenAt.atStartOfDay() until LocalDate.now().atStartOfDay() <= 2.days
                    }

                if (allowNotifications && gradesEligibleForNotification.isNotEmpty()) {
                    if (gradesEligibleForNotification.size == 1) {
                        ...
                        val schulverwalterUserIdsThatGotNewGrades =
                            gradesEligibleForNotification.map { it.schulverwalterUserId }.toSet()
            ...
            besteSchuleApiRepository.setYearForUser(user.schulverwalterConnection!!.accessToken, null)
        }
    }
}