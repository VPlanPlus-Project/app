...
class UpdateProfileAssessmentIndexUseCase(
    ...
    suspend operator fun invoke(profile: Profile) {
        assessmentRepository.dropIndexForProfile(profile.id)
        assessmentRepository.getAll()
            ...
            .filter { assessment ->
                (assessment.creator is AppEntity.Profile && (assessment.creator as AppEntity.Profile).profile.id == profile.id) ||
                        ...
                        (assessment.subjectInstance.id in (profile as? Profile.StudentProfile)?.subjectInstanceConfiguration.orEmpty().filterValues { it }.keys.map { it.id })
            }
