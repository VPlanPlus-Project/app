...
class ImportScreenViewModel(
    ...
    fun handleEvent(event: ImportScreenEvent) {
        viewModelScope.launch {
            when (event) {
                is ImportScreenEvent.UpdateText -> {
                    state = state.copy(text = event.text)
                    if (!allowInit) return@launch
                    readMigrationDataUseCase(event.text).let { result ->
...
sealed class ImportScreenEvent {
    data class UpdateText(val text: String) : ImportScreenEvent()
}