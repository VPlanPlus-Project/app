...
class HomeworkSource(
    ...
    override fun getAll(configuration: FetchConfiguration<Homework>): Flow<List<Cacheable<Homework>>> {
    ...
    override fun getById(
        ...
        homeworkRepository.getById(id.toInt()).collectLatest { cacheableHomework ->
            ...
            if (configuration is FetchConfiguration.Ignore || cacheableHomework !is Cacheable.Loaded) return@collectLatest
            if (cacheableHomework.value is Homework.CloudHomework && configuration is Homework.Fetch) {
                if (configuration.vppId is FetchConfiguration.Fetch) {
