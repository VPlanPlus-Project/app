            ...
            try {
                run schulverwalterConnectionRefresh@{
                    val invalidVppIds = vppIdRepository
                        ...
                        .mapNotNull { vppId ->
                            val isValid =
                                besteSchuleRepository.checkValidity(vppId.schulverwalterConnection!!.userId)
                    ...
                    invalidVppIdsAfterTokenReload.forEach { stillInvalidVppId ->
                        val vppId =
                            vppIdRepository.getById(stillInvalidVppId).first() as? VppId.Active
                                ?: return@forEach
                        if (vppId.schulverwalterConnection!!.isValid == false) return@forEach
                        besteSchuleRepository.saveBesteSchuleAccessValidity(
                            vppId.schulverwalterConnection!!.userId,
                            false
                        )
                    ...
                    (invalidVppIds - invalidVppIdsAfterTokenReload).forEach { nowValidVppId ->
                        val vppId = vppIdRepository.getById(nowValidVppId).first() as? VppId.Active
                            ?: return@forEach
                        besteSchuleRepository.saveBesteSchuleAccessValidity(
                            vppId.schulverwalterConnection!!.userId,
                            true
